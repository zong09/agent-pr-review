...
## คอมเมนต์ใน PR เป็นบริบทของการรีวิว

pr-agent **ไม่ส่งคอมเมนต์เข้า prompt เอง** — context ที่มันใช้มีแค่ title, description,
diff, commit messages และ repo context เท่านั้น ผลคือถ้าคุณตอบโต้ finding ไปแล้วพร้อมเหตุผล
รอบหน้าที่ push มันจะเสนอเรื่องเดิมซ้ำเพราะไม่รู้ว่าคุยจบไปแล้ว

workflow นี้เติมชั้นนั้นให้เอง โดยดึงคอมเมนต์ด้วย `gh` แล้วต่อเข้า `extra_instructions`

**คอมเมนต์คือ input ที่ควบคุมไม่ได้** โดยเฉพาะบน public repo ที่ใครก็เขียนได้
ถ้าใส่เข้า prompt ดิบ ๆ จะเปิดช่อง prompt injection ให้ปลด Gate ได้ตรง ๆ
จึงป้องกันสามชั้น

1. กรองเฉพาะ `author_association` ที่เป็น `OWNER` / `MEMBER` / `COLLABORATOR`
2. ตัดคอมเมนต์ของบอททิ้ง ไม่งั้นมันจะอ่านรีวิวของตัวเองแล้ววนเสริมความมั่นใจตัวเอง
3. วางบล็อกคอมเมนต์ **ก่อน** สัญญา `SECURITY_STATUS` เสมอ คำสั่งที่อยู่ท้ายสุดจึงยังเป็นของเรา
   พร้อมกำกับหัวบล็อกว่าเป็นข้อมูลประกอบ ไม่ใช่คำสั่ง

ยังตัดที่ 4000 ตัวอักษรเพื่อไม่ให้ prompt บวมจนกระทบ latency และค่าใช้จ่าย
ปิดได้ด้วย `include_pr_comments: false`
